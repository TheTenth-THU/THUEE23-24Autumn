...
% \chapter*{第三稿说明}
% %%%%%%%%%%%%%%%%%%%%%
% \addcontentsline{toc}{section}{第三稿说明}

% 在第二稿的基础上，我组织演员们逐段分析了剧本，对每一段的情节、人物、台词等进行了讨论。在这个过程中，演员们提出了许多宝贵的意见，对剧本的细节进行了修改，使得剧本更加符合实际情况，也更加生动有趣。
...
%%%%%%%%%%%
	\scene{
		...
		\dialog[AC]{
			\direction{不耐烦地} 要不就这样，差不多得了。 \direction{把说明书丢到一边}
		}
		\dialog[B0]{
			再看看呢，也没几个地方要检查了。\direction{拿起说明书}
		}
		\dialog[AC]{
			\direction{一脸厌倦、无奈} 何必呢。就算能得到结果，也得先准备答辩去了。
		}
		\dialog[B0]{
			但是连一个像样的信号都没收到，答辩咱能说啥呢？
		}
		\dialog[B1]{
			给评委们谈谈背景，谈谈理想，谈谈愿景嘛。\direction{装模作样起来}咳咳，「我们的设备虽然没收到可识读的信号，但这只是暂时的技术性调整，我们的项目仍然前景远大、前途光明，背后是……」
		}
		\dialog[AC]{
			\direction{打断} 你打住！\direction{笑} 说起来，咱们这个项目最初是要干啥来着？
		}
		\dialog[B0]{
			最初你说要做一个通信终端，然后几百块的报销额度只能做一端，所以就只做了一个接收端。
		}
		\dialog[B1]{ 
			然后只有接收端就没有办法控制测试信号，所以就只有在天台上看微波背景辐射了。
		}
		\dialog[AC]{
			但是就我们手上这些东西 \direction{把黑盒盖子打开}，板子是串口不够刚换的，线头是断了又重新焊的，淘宝买的芯片问客服还说测试程序是外包的，又怎么捕获到那么微小的辐射波动呢？
		...
		\dialog[B1]{
			从另一个角度想，我们要解决的问题不也没多少嘛。\direction{推开盖子，开始检查} 反正也就这些接线、程序、电源、天线是我们能控制的。看到一点比现在的噪声大的波动，我们就算成功了。
		}
		\stage{
...
%%%%%%%%%%%
	\scene{
		\stage{
			\textup{
				\textbf{时间点\(\boldsymbol{P}\)}，T大学机房。
			}
			渐渐入夜，参与科技营带班授课的几位同学正在一台一台地手动安装软件，为即将开始的教学活动做准备。机房窗外，刚来到 T 大学的 AP 带着D组同学们背着包、抱着书，靠在窗边向里张望。
		}
		\dialog[C1]{
	...
	\scene{
		\stage{
			...
			机房内，C 组同学为 AP 以及 D 组同学们讲解着单片机的基本原理和编程方法。
		}
		\stage{
			【本段以闪过教学片段的方式呈现】
		}
	...
	\scene{
		\stage{
			\textup{
				\textbf{时间点\(\boldsymbol{P}\)}，T大学机房附近的研讨间。
			}
			进入自主设计阶段，AP 和同组的同学在 C0 的带领下来到一个研讨间，兴奋地讨论起他们的设计方案。这一组的指导助教 C0 在一旁默默地听着，同时在电脑上调试着自己的程序。
		}
		\dialog[D0]{
			\direction{摆弄着几个舵机} 
			你们说，拿这几个舵机做个按摩椅模型，怎么样？
			\direction{拿起舵机，吸引注意}
			这个，这个！做按摩椅！
		}
		\dialog[D1]{
			不大行吧，刚刚老师不是说这个舵机工作一段时间就会抽风嘛。
		}
		\dialog[D3]{
			而且，只做一个模型，也没有实际功能，不太好哦。
		}
		\dialog[D2]{
			那做个闹钟呢？
		}
		\dialog[D4]{
			欸，把舵机放在闹钟上，早上闹钟响了，舵机就——
		}
		\dialog[D1]{
			——把你的被子给扯掉，这样就不会再睡过头了。\direction{众笑}
		}
		\dialog[D3]{
			这小舵机哪有那么大劲啊。
			或者做个收纳箱吧，把舵机放在箱子里，然后用板子控制箱子的开合——
		}
		\dialog[D2]{
			——用摇杆「上上下下左左右右AABB」当密码，\direction{众笑}欸，有用处。
		}
		\dialog[AP]{
		...
		\dialog[AP]{
			\direction{灵光一闪，拍案而起} 不是只够做一个终端的吗？干脆我们直接把自己的信息编码成二进制串，然后发射到太空中去！
		}
		\stage{
			沉默。
		}
	...
	\scene{
		\stage{
			\textup{
				\textbf{时间点\(\boldsymbol{P}\)}，T大学操场。
			}
			夜色已深。操场上空无一人，只有几盏昏黄的路灯洒在湿漉漉的地面上。AP 思索着自己的设计方案，看着远处的天空，更加坚持自己的想法，但既不确定其是不是真的可行，也不知道如何说服同组的同学。AP 在操场上独自思考。
		}
		\stage{
			C0 从教学楼走出，看到操场上的 AP 正在思索些什么，便走了过去。
		}
		\dialog[C0]{
			\direction{走到 AP 身边} 怎么，这么晚了还在这儿？
		}
		\dialog[AP]{
			\direction{抬起头，勉强挤出一个笑容，看向远方} 我在想，我的设计方案，是不是真的太大了？
		}
		\dialog[C0]{
			\direction{扭头，思考} 你说的是那个……把信息编码成二进制串，然后发射到太空的想法吗？
		}
		\dialog[AP]{ 
			\direction{回头} 嗯。\direction{开始踱步}
		}
		\dialog[C0]{
			\direction{跟上脚步} 你这个想法确实挺大胆的，设计的东西也可能比较复杂。
			嗯，不过有我们支教队的同学在这，这方面你倒是不用太担心。
			哎，\direction{抬头} 对了，你有想过，这个方案的意义吗？
		}
		\dialog[AP]{
			\direction{思考} 意义？\direction{摇头}
		}
		\dialog[C0]{
			我们之前做硬件设计的时候，我们组做的是一个比较愚蠢的温控风扇，还算是有点实际用途；我有一群同学做的智能药盒，能提醒老年人按时吃药……你有类似的想法吗？
		}
		\dialog[AP]{
			\direction{低头} 我……我不确定，但我的意义好像跟这些不太一样。
		}
		\dialog[C0]{
			没事，方案复杂了我们可以帮你解决，我们支教队的同学在这儿呢。
		}
		\dialog[AP]{
			\direction{点头} 嗯。
		}
		\dialog[C0]{
			意义的话，\direction{停顿}其实很多很成功的事情，最开始也没有什么明确的意义和目的。没事的，如果你想坚持的话，就大胆去做吧。
			\direction{patpat}回去吧，明天还有课呢，你也需要休息。
		}
		\dialog[AP]{
			\direction{看着远方，思考} 嗯。\direction{出场}
		}
	}
	\scene{
		\stage{
			...
			第二天，C0 带着 AP 和其他组员们再次聚集在研讨间。C0 主动提起了 AP 的想法，拿出了一夜论证的结果，并鼓励大家一起去尝试和实现。同学们也被 C0 的鼓励所感染，纷纷在这个方案的基础上提出了更多的想法，开始了他们的设计和实现。
		}
		\dialog[C0]{
			\direction{拿出一沓纸} 昨晚我想了一晚上，AP 提出的这个方案，我觉得还是有一定的可行性的。
			更重要的是，这个方案具有很强的创新性和突破性，也能让我们学到很多东西。
			我们可以先从这个方案的基本框架出发，逐步完善，逐步实现。
		}
		\dialog[AP]{
			在宇宙中留下我们的痕迹，这个想法也太酷了吧！
		}
		\stage{
			同学们在稿纸上写写画画，讨论着各自的想法，C0 也在一旁指导着。随着时间推移，接线、焊接、编程、调试、封装，一切都在有条不紊地进行着，直到结营展示的到来。
		}
		\stage{
			【本段以闪过设计讨论片段的方式呈现】
		}
...
%%%%%%%%%%%
	\scene{
		...
		\dialog[B0]{
			这个波……反正不是预期。还是哪有问题，要不还是老办法，Reset一下？
		}
		\stage{
			B0 正要伸手去按 Mega 板的 Reset 键，AC 突然（有些激动地）拦住了他。
		}
		\dialog[AC]{
			\direction{激动} 不要！这个波……先让它把收到的数据记着！如果我没猜错的话，马上，你们马上就知道我在说什么了！
		}
		\stage{
			（在激昂的BGM下）AC 快速敲起代码，凭着印象中当年的编码方式复现出了一个解码程序。绘图仪中的图像慢慢平缓下来，突然出现的信号消失了，只剩下噪声起起伏伏。
		}
		\stage{
		...
		\dialog[AC]{
			\direction{仿佛在自言自语} 这是……这是之前的编码…… \direction{缓过神来} 
			我高中的时候，就用这种编码方式，把这首诗发到天上去过。
		}
		\dialog[B1]{
			等等等等，这首诗？几年前？发到天上？\direction{大为震撼}
		}
		\dialog[B0]{
			\direction{大为震撼} 你是说，这个信号从那时候就一直在空间中传播，还折回到了现在？
		}
		\dialog[B1]{
			这真的不是只有在科幻 DV 剧里才会出现的情节吗？
		}
		\dialog[AC]{
			\direction{难以置信} 我也不知道，但这个信号，它确确实实就在这里。其实，我之前都一直不敢确定，我当年的那个想法，到底是不是真的实现了。
		}
		\stage{
			AC 解释着自己的难以置信却又铁证如山的想法，感觉到了当年的那份探索热情。冥冥之中，似乎确实有什么东西在等待着她。这种意义感让她重新燃起了对这个项目的热情，她不由自主得想要发个朋友圈。
		}
		\dialog[B0]{
			\direction{打断} 快开工吧，这至少说明我们电路没有问题了。至于这个特殊的信号从哪里来，我们以后再研究嘛。 
		}
		\dialog[AC]{
			\direction{点头} 嗯，对，对。 \direction{开始调试电路}
		}
	}
	\scene{
		\stage{
			\textup{
				\textbf{时间点\(\boldsymbol{C}\)}，T大学会议室。
			}
			时隔几天，AC 和 B0、B1 三人在会议室里，向老师们汇报了他们的硬件设计成果。这一段时间，他们不断地调试、改进，终于实现了一个稳定的通信终端，取得了理想的成绩。
		}
		\stage{
			本段以闪过汇报片段的方式呈现：
			\begin{itemize}
				\item 三人捧着自己的通信终端，向评委们展示。
				\item 评委席上的评委向三人提问。（侧脸）
				\item 三人鞠躬，向评委们致谢。
				\item 三人端起作品和奖状自拍庆祝。
			\end{itemize}
		}
		...
		\dialog[评委2（画外音）]{
			后续的改进方向，你们有没有考虑过？我建议你们去找 L 老师，他对这方面的研究很深。
		}
		\dialog[主持人（画外音）]{
			获得特等奖的作品是——AC、B0、B1 同学的「BeyondTheFrequency 通信终端」！
		}
	}
	\scene{
		\stage{
			...
			实践支队来到 P 中学的支教活动前夜，包括 AC 在内的几位同学正在一台一台地手动安装软件，为即将开始的支教活动做准备。窗外，刚下晚自习的 E 组同学们背着包、抱着书，靠在窗边向里张望。里面的同学看向窗外，AC 如第二幕第一场的 C0 一样走到窗前。窗外的同学们看到被发现后，有些尴尬地哄笑了一下，相视一笑，把带他们来的
			E0 推到了门前，哄笑着跑开到远处。队员们看着 E0 的背影，笑了一下，AC 前去拉住了 E0，带他进了机房。
		}
		\dialog[AC]{
			怎么，是好奇我们在干嘛吗？
		}
		\stage{
			【本段的作用是且仅是呼应第二幕第一场，不做更多展开】
		}
	}
}
...
%%%%%%%%%%%
	\scene[创作背景介绍]{
		\dialog[字幕]{
			本剧中所提及的所有作品，除主角的通信终端外，均为真实作品。
		}
		\stage{
			展示三字班硬件设计大赛的优秀作品（如平衡小车、智能药盒等），以及支教队指导高中生的优秀作品（如闹钟、音乐盒、按摩椅等）。
		}
	}
}
