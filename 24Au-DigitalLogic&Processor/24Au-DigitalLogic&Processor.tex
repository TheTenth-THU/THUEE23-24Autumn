...
\bu[传输门]{
	...
	传输门是一种多输入单输出的逻辑门，其输出为与或逻辑（AND-OR）运算的结果。
}
...
用 \(m\) 个二进制位对 \(n \le 2^m\) 个输入信号进行编码，得到 \(m\) 位二进制代码的电路。

\paragraph{译码器（Decoder）}

用 \(n\) 个二进制位对 \(m \le log_2 n\) 个输入信号进行译码，得到 \(n\) 位二进制代码的电路。

\paragraph{多路选择器（Multiplexer，MUX）}

用 \(n\) 个控制信号对 \(2^n\) 个输入信号进行选择，得到一个输出信号的电路。
...
\begin{example}
	设计一个 4-bit 全加器电路。
	\begin{solution}
		1-bit 全加器有 3 个输入信号（ \(A\)、\(B\) 和进位信号 \(C_\mathrm{in}\)）、2 个输出信号（本位的和 \(S\) 、进位输出信号\(C_\mathrm{out}\)，其真值表如表~\ref{Tab: 1-bit Full Adder}~所示。

		\captionof{table}{1-bit 全加器真值表}
